...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///blog.db'  # 適切なデータベースURIを設定
...
login_manager.init_app(app) #紐づけ
...
def index():
    if request.method=="GET":
        ...
        if text_input is None or len(text_input) == 0:
            ...
            posts = db.session.query(Post).filter(or_(Post.body.contains(text_input), Post.title.contains(text_input))).all()
